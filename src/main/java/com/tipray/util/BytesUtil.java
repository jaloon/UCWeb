...
public class BytesUtil {
    ...
    private static final char[] BINARY_DIGEST = {'0', '1'};
    ...
    public static String bytesToHex(byte[] bytes, boolean toLowerCase) {
        ...
        char[] hexArray = toLowerCase ? HEX_DIGITS_LOWER : HEX_DIGITS_UPPER;
    ...
    public static String bytesToBinaryString(byte[] bytes) {
        ...
        char[] binaryChars = new char[bytes.length * 8];
        for (int i = 0; i < bytes.length; i++) {
            byte b = bytes[i];
            binaryChars[i * 8] = BINARY_DIGEST[(b >> 7) & 1];
            binaryChars[i * 8 + 1] = BINARY_DIGEST[(b >> 6) & 1];
            binaryChars[i * 8 + 2] = BINARY_DIGEST[(b >> 5) & 1];
            binaryChars[i * 8 + 3] = BINARY_DIGEST[(b >> 4) & 1];
            binaryChars[i * 8 + 4] = BINARY_DIGEST[(b >> 3) & 1];
            binaryChars[i * 8 + 5] = BINARY_DIGEST[(b >> 2) & 1];
            binaryChars[i * 8 + 6] = BINARY_DIGEST[(b >> 1) & 1];
            binaryChars[i * 8 + 7] = BINARY_DIGEST[b & 1];
        }
        return new String(binaryChars);
    }

    /**
     * byte数组转换为二进制字符串，每个字节之间使用指定分隔符分隔
     *
     * @param bytes     byte数组
     * @param separator 每个字节间分隔符
     * @return 二进制字符串
     */
    public static String bytesToBinaryString(byte[] bytes, char separator) {
        if (bytes == null) {
            return "";
        }
        ...
        for (int i = 0; i < bytes.length; i++) {
            byte b = bytes[i];
            binaryChars[i * 9] = BINARY_DIGEST[(b >> 7) & 1];
            binaryChars[i * 9 + 1] = BINARY_DIGEST[(b >> 6) & 1];
            binaryChars[i * 9 + 2] = BINARY_DIGEST[(b >> 5) & 1];
            binaryChars[i * 9 + 3] = BINARY_DIGEST[(b >> 4) & 1];
            binaryChars[i * 9 + 4] = BINARY_DIGEST[(b >> 3) & 1];
            binaryChars[i * 9 + 5] = BINARY_DIGEST[(b >> 2) & 1];
            binaryChars[i * 9 + 6] = BINARY_DIGEST[(b >> 1) & 1];
            binaryChars[i * 9 + 7] = BINARY_DIGEST[b & 1];
            binaryChars[i * 9 + 8] = separator;
        }
    ...
    public static String byteToBinaryString(byte b) {
        ...
        binaryChars[0] = BINARY_DIGEST[(b >> 7) & 1];
        binaryChars[1] = BINARY_DIGEST[(b >> 6) & 1];
        binaryChars[2] = BINARY_DIGEST[(b >> 5) & 1];
        binaryChars[3] = BINARY_DIGEST[(b >> 4) & 1];
        binaryChars[4] = BINARY_DIGEST[(b >> 3) & 1];
        binaryChars[5] = BINARY_DIGEST[(b >> 2) & 1];
        binaryChars[6] = BINARY_DIGEST[(b >> 1) & 1];
        binaryChars[7] = BINARY_DIGEST[b & 1];
        return new String(binaryChars);
    }
