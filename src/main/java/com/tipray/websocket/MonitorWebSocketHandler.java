...
public class MonitorWebSocketHandler implements WebSocketHandler {
	...
	private void dealFocusMonitor(WebSocketSession session, Map<String, Object> msgMap) {
		...
		case BIZ_REQUEST:
			...
			try {
				...
				// TODO
				sendMsg(session, "解析监控指令异常，请查证指令格式是否正确");
				return;
			}
			...
			int cacheId = 0;
			try {
				cacheId = addUdpCache(UdpBizId.FOCUS_MONITOR_REQUEST, description);
				// 车载终端设备ID
				...
				try {
					...
					if (cacheId != 0) {
						AsynUdpCommCache.putLogCache(cacheId, vehicleManageLog.getId());
					}
				} catch (Exception e) {
					logger.error("添加车辆管理日志：\nVehicleManageLog = {}。\n异常：{}", vehicleManageLog, e.toString());
				}
			}
			break;
		case BIZ_REPEAT:
	...
	private void dealFocusCancel(Map<String, Object> msgMap) {
		...
		if (carId != null && carId != 0) {
			...
			if (terminalId != null && terminalId != 0) {
				...
				int cacheId = 0;
				try {
					cacheId = addUdpCache(UdpBizId.FOCUS_MONITOR_CANCEL_REQUEST, description);
	...
	private void dealRealtimeMonitor(WebSocketSession session, Map<String, Object> msgMap) {
		...
		case BIZ_REQUEST:
			...
			cars.parallelStream().forEach(car -> {
				...
				int cacheId = 0;
				try {
					cacheId = addUdpCache(UdpBizId.REALTIME_MONITOR_REQUEST, description);
					// 车载终端设备ID
		...
		case BIZ_CANCEL:
			...
			if (!EmptyObjectUtil.isEmptyList(ids)) {
				ids.parallelStream().forEach(carId -> dealRealtimeCancel(user, carId, cancelSessionId));
			}
			break;
		default:
			break;
	 ...
	 *            {@link WebSocketSession#getId()}
	 */
	private void dealRealtimeCancel(User user, Long carId, Long sessionId) {
		if (carId != null && carId != 0) {
			// 车载终端设备ID
			Integer terminalId = vehicleService.getTerminalIdById(carId);
			if (terminalId != null && terminalId != 0) {
				...
				int cacheId = 0;
				try {
					cacheId = addUdpCache(UdpBizId.REALTIME_MONITOR_CANCEL_REQUEST, description);
	 ...
	 * 
	 * @param status
	 *            状态值（0：未知 | 1：在油库 | 2：在途中 | 3：在加油站 | 4：返程中 | 5：应急）
	 * @return 车辆状态名称
	 */
	 ...
	 * 是否报警
	 * 
	 * @param carAlarmStatus
	 *            车辆报警状态
	 * @param lockStatusInfoHex
	 *            锁状态16进制字符串
	 * @return
	 */
	 ...
	 */
	private int addUdpCache(short bizId, String description) {
	 ...
	 *            {@link String} 请求发起结果
	 */
	public void broadcastRemoteControl(String task, String result) {
